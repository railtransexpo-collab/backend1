...
Thank you for submitting your application form to participate as an awardee at the 6th RailTrans Expo 2026.
...
<p>Thank you for submitting your application form to participate as an <strong>awardee</strong> at the <strong>6th RailTrans Expo 2026</strong>.</p>
  ...
  try {
    ...
    (async () => {
      try {
        ...
        console.error("[awardees] Notification email failed:", e);
      }
    })();
    ...
    (async () => {
      try {
        const adminEnv =
          process.env.AWARDEE_ADMIN_EMAILS || process.env.ADMIN_EMAILS || "";
        ...
        console.error(
          "[awardees] admin notify error (non-fatal):",
          e && (e.stack || e),
        );
      }
    })();
...
router.post("/:id/resend-email", async (req, res) => {
  try {
    ...
    if (!doc)
      return res
        .status(404)
        .json({ success: false, error: "Awardee not found" });
    if (!doc.email)
      return res
        .status(400)
        .json({ success: false, error: "No email found for awardee" });
    ...
    try {
      ...
      return res
        .status(500)
        .json({ success: false, error: "Failed to resend ack email" });
    }
  } catch (e) {
...
router.post("/:id/send-ticket", async (req, res) => {
  try {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ success: false, error: "Failed to send ticket email" });
    }
  } catch (e) {
...
router.get("/", async (req, res) => {
  try {
    ...
    const flattened = rows.map((r) => {
      ...
      if (output.createdAt instanceof Date)
        output.createdAt = output.createdAt.toISOString();
      if (output.updatedAt instanceof Date)
        output.updatedAt = output.updatedAt.toISOString();
      return output;
    });
...
`;
        ...
        await mailer.sendMail({
          ...
          from:
            ...
            "RailTrans Expo <support@railtransexpo.com>",
        });
    ...
    console.error(
      "[awardees] send-reminders error:",
      err && (err.stack || err),
    );
    return res
      .status(500)
      .json({ success: false, error: "Failed to send reminders" });
  }
});
...
router.get("/stats", async (req, res) => {
  try {
    ...
    const paid = await db
      .collection("awardees")
      .countDocuments({ txId: { $exists: true, $ne: null, $ne: "" } });
    const free = await db
      .collection("awardees")
      .countDocuments({ ticket_category: { $regex: /(free|general|^0$)/i } });
...
router.post("/:id/confirm", express.json(), async (req, res) => {
  try {
    ...
    if ("ticket_code" in updateData) {
      const incoming = updateData.ticket_code
        ? String(updateData.ticket_code).trim()
        : "";
      const existingCode = existing.ticket_code
        ? String(existing.ticket_code).trim()
        : "";
    ...
    console.error(
      "[awardees] POST /:id/confirm (mongo) error:",
      err && (err.stack || err),
    );
    return res
      .status(500)
      .json({ success: false, error: "Failed to update awardee" });
  }
});
/**
 * PUT /api/awardees/:id
...
router.post("/:id/upload-proof", upload.single("proof"), async (req, res) => {
  try {
    ...
    await db
      ...
      .updateOne(
        { _id: oid },
        { $set: { proof_path: proofPath, updatedAt: new Date() } },
      );
    ...
    console.error(
      "[awardees] upload-proof (mongo) error:",
      err && (err.stack || err),
    );
    return res
      .status(500)
      .json({ success: false, error: "Failed to upload proof" });
  }
});
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    console.error(
      "[awardees] DELETE (mongo) error:",
      err && (err.stack || err),
    );
    return res
      .status(500)
      .json({ success: false, error: "Failed to delete awardee" });
  }
});
...
router.get("/download-all", async (req, res) => {
  try {
    ...
    const docs = await db.collection("awardees").find({}).toArray();
    
    if (!docs || docs.length === 0) {
      return res.status(404).json({ success: false, error: "No records found" });
    }

    res.attachment("Awardees-Badges.zip");
    const archive = archiver("zip", { zlib: { level: 9 } });
    archive.pipe(res);

    for (const doc of docs) {
      try {
        const pdf = await generateBadgePDF("awardee", doc, { mode: "email" });
        archive.append(pdf, { 
    ...
    console.error("[awardees] download all badges error:", error);
...
router.get("/:id/download-badge", async (req, res) => {
  try {
    ...
    const doc = await db.collection("awardees").findOne({ _id: oid });
    if (!doc) {
      return res.status(404).json({ success: false, error: "Awardee not found" });
    }

    const pdfBuffer = await generateBadgePDF("awardee", doc, { mode: "email" });
    ...
    console.error("[awardees] download badge error:", error);
    res.status(500).json({ success: false, error: error.message });
  }
});
