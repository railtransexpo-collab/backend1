...
Thank you for submitting your application form to participate as a speaker at the 6th RailTrans Expo 2026.
...
<p>Thank you for submitting your application form to participate as a <strong>speaker</strong> at the <strong>6th RailTrans Expo 2026</strong>.</p>
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    if (Array.isArray(payload.slots) && payload.slots.length > 0) {
      ...
      slotsValue = Array.isArray(payload.slot_duration)
    ...
    const doc = {
      ...
      registered_at: payload.registered_at
        ? new Date(payload.registered_at)
        : new Date(),
      admin_created_at: payload.added_by_admin
        ? new Date(payload.admin_created_at || Date.now())
        : undefined,
    };
    ...
    doc.ticket_code =
      payload.ticket_code || payload.ticketCode
        ? String(payload.ticket_code || payload.ticketCode)
        : generateTicketCode();
    ...
    if (doc.ticket_code && r && r.insertedId) {
      try {
        await col.updateOne(
          { _id: r.insertedId },
          { $set: { ticket_code: doc.ticket_code } },
        );
      } catch (e) {
        /* ignore */
      }
    ...
    (async () => {
      try {
        ...
        console.error("[speakers] Notification email failed:", e);
      }
    })();
    ...
    return res
      .status(500)
      .json({ success: false, error: "Failed to create speaker" });
  }
});
...
router.post("/:id/resend-email", async (req, res) => {
  try {
    ...
    if (!isEmailLike(doc.email))
      return res
        .status(400)
        .json({ success: false, error: "No valid email found for speaker" });
    ...
    console.error(
      "POST /api/speakers/:id/resend-email error:",
      err && (err.stack || err),
    );
    return res
      .status(500)
...
router.post("/:id/send-ticket", async (req, res) => {
  try {
    ...
    console.error(
      "POST /api/speakers/:id/send-ticket error:",
      err && (err.stack || err),
    );
    return res
      .status(500)
...
router.get("/:id", async (req, res) => {
  try {
    ...
    console.error(
      "GET /api/speakers/:id (mongo) error:",
      err && (err.stack || err),
    );
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: "Failed to delete speaker" });
  }
});
